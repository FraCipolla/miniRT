...
/*   Updated: 2022/08/30 16:40:45 by mcipolla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	computeLighting(double *P, double *N, t_data *data, t_obj *closest)
{
	...
	{
		L = sub_vec(data->light.pos.vec, P);
		dot_l = dot(N, L);
		if (dot_l > 0)
	...
	if (closest->specular != -1)
	{
		...
		double r_dot_v = dot(R, mult_vec_n(data->cam.dir.vec, -1));
		if (r_dot_v > 0)
		{
			i += data->light.bright * pow(r_dot_v / (norm(R) * norm(mult_vec_n(data->cam.dir.vec, -1))), closest->specular);
		}
	}
	if (i > 1)
		i =	1;
