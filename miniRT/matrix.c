...
/*   Updated: 2022/09/04 18:54:52 by mcipolla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	**rot_matrix(t_v3 rot, t_v3 dir)
{
	...
	matrix[0][1] = cos(rot.z * M_PI) * sin(rot.x * M_PI) * sin(rot.y * M_PI) - cos(rot.x * M_PI) * sin(rot.z * M_PI);
	matrix[0][2] = cos(rot.x * M_PI) * cos(rot.y * M_PI) * sin(rot.x * M_PI) + sin(rot.x * M_PI) * sin(rot.z * M_PI);
	matrix[1][0] = cos(rot.y * M_PI) * sin(rot.z * M_PI);
	matrix[1][1] = cos(rot.x * M_PI) * cos(rot.y * M_PI) + sin(rot.x * M_PI) * sin(rot.y * M_PI) * sin(rot.z * M_PI);
	matrix[1][2] = cos(rot.x * M_PI) * sin(rot.y * M_PI) * sin(rot.z * M_PI) - cos(rot.z * M_PI) * sin(rot.x * M_PI);
